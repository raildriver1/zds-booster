...
// Формат значения с точкой как разделителем — не зависит от локали.
function FormatFloatDotLocal(Value: Single): string;
var
  OldSep: Char;
begin
...
// Парсинг дробного значения из конфига, устойчивый к локали:
// принимает и точку, и запятую, не бросает исключений.
function ParseFloatDotLocal(const S: string; DefaultValue: Single): Single;
var
  OldSep: Char;
  Normalized: string;
begin
...
// Проверяет, начинается ли строка с "key:" (игнорируем регистр и пробелы)
function LineHasKey(const Line, Key: string): Boolean;
var
  Trimmed: string;
  ColonPos: Integer;
begin
  Result := False;
  Trimmed := SysUtils.Trim(Line);
  ColonPos := Pos(':', Trimmed);
  if ColonPos > 0 then
    Result := SysUtils.Trim(Copy(Trimmed, 1, ColonPos - 1)) = Key;
end;
...
var
  ...
  begin
    ...
    for j := 0 to Lines.Count - 1 do
      if LineHasKey(Lines[j], Key) then
      begin
        Lines[j] := Full;
        Exit;
      end;
...
begin
  ...
  try
    ...
    UpsertLine('basespeed',          FormatFloatDotLocal(MenuFreecamBaseSpeed));
    UpsertLine('fastspeed',          FormatFloatDotLocal(MenuFreecamFastSpeed));
    UpsertLine('turnspeed',          FormatFloatDotLocal(MenuFreecamTurnSpeed));
    UpsertLine('stepforward',        FormatFloatDotLocal(stepforward));
