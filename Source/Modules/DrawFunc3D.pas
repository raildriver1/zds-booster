...
begin
  try
    ...
    // Патчим Launcher.exe+92258:
    // Меняем fcomp dword ptr [Launcher.exe+942CC] -> fcomp dword ptr [$00791DD8]
    // fcomp = D8 1D [адрес]
    InstructionAddress := Pointer($492258); // Launcher.exe+92258

    // Снимаем защиту на запись
    ...
    begin
      ...
      Inc(InstructionAddress);
      PDWORD(InstructionAddress)^ := $00791DD8; // пишем адрес как DWORD
