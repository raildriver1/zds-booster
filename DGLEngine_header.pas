...
ProcessKlubKeys: procedure(); stdcall;
...
HookKLUB : procedure (KranGR: ShortInt = 0; KombKr: ShortInt = 0; x: Single = 0; y: Single = 0; z: Single = 0; AngZ: Single = 0); stdcall;
ProcessKlubKeys: procedure(); stdcall;
DrawCabElement_10 : procedure(
  ModelID: Integer;      // eax -> сохраняется в локальной переменной
  TextureID: Cardinal;   // edx -> используется потом  
  Color: Integer;        // ecx -> сохраняется в локальной переменной
  Smooth: Boolean;       // [ebp+08]
  Alpha: Byte;           // [ebp+10] 
  Frame: Integer;        // [ebp+14]
  Scale: Double;         // [ebp+18]
  AngZ: Double;          // [ebp+20]
  AngY: Double;          // [ebp+28] 
  AngX: Double;          // [ebp+30]
...
HookText : procedure (Font : Cardinal; X,Y : integer; Text : string; Color : integer; Alpha : integer = 255; Scale : real = 1.0); stdcall;
...
begin
  ...
  @HookKLUB := GetProcAddress(DGLEngineDLL_Handle, 'HookKLUB');
  @ProcessKlubKeys := GetProcAddress(DGLEngineDLL_Handle, 'ProcessKlubKeys');
  @HookText := GetProcAddress(DGLEngineDLL_Handle, 'HookText');
  @DrawCabElement_10 := GetProcAddress(DGLEngineDLL_Handle, 'DrawCabElement_10');
